...
export class NewColorModal extends Component<NewColorModalProps, NewColorModalState> {
    ...
    private updatePresetIndex: EventListener = (e: Event) => {
    ...
    private renderPresetButton = (color: Color, index: number) => (
        <button
            className={`preset-button ${index === this.state.presetIndex ? 'is-selected' : ''}`}
            data-index={index}
            onClick={this.updatePresetIndex}
            style={{ backgroundColor: color.hex, color: contrast(color.hex, '#000000') >= 4.5 ? '#000000' : '#ffffff' }}
        >
            {color.hex}
        </button>
    );
    ...
    render() {
        ...
        if (this.state.tab === 'custom') {
            ...
            view = this.presets.map((color: Color, index: number) => this.renderPresetButton(color, index));
        }
        return (
